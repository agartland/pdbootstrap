...
def _bootstrap_jit(dat, statfunction, indices, nstraps, nstats):
    res = np.zeros((nstraps, nstats))
    # Fast path: if statfunction is None, compute mean directly
    if statfunction is None:
        for booti in prange(nstraps):
            rind = indices[booti]
            # Single column mean
            res[booti, 0] = np.mean(dat[rind, 0])
    else:
        for booti in prange(nstraps):
            rind = indices[booti]
...
def bootci_nb(dat, statfunction, alpha=0.05, n_samples=10000, method='bca'):
    ...
    # Generate all bootstrap indices outside JIT
    n = dat.shape[0]
    indices = np.random.randint(0, n, size=(n_samples, n))
    # If statfunction is a simple mean, use fast path
    fast_mean = False
    if hasattr(statfunction, "__name__") and statfunction.__name__ == "stat_mean_single_col":
        fast_mean = True
    boot_res = _bootstrap_jit(dat, None if fast_mean else statfunction, indices, nstraps=n_samples, nstats=nstats)
